...
export const proxyRequest = async (req: Request, res: Response, next: NextFunction) => {
  try {
    // 🛡️ Safe Path Mapping: Gateway /api/blood-bank -> Microservice /blood-bank
     ...
     const cleanedPath = req.originalUrl.replace("/api/hospital", "/hospital").replace("/api/prescription-template", "/prescription-template").replace("/api/attendances", "/attendances").replace("/api/devices", "/devices").replace("/api/fingerprint-enrollments", "/fingerprint-enrollments");
